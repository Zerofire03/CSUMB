...
def enter():
    enterhouse = raw_input("Do you want to play a game?  Great rewards lie inside! - press 'y' to continue: ")
...
def roomone():
  ...
  while true:
    direction = raw_input("Please choose a room (lt) will take you to The Room of The Deep, (fd) will take you to the Room of Questionnaire, (rt) will take you to the Locker Room, (bk)will take you back they way you entered the game, or (vw) to view the object in the center of the room: ")
    if direction == "rt":
      room2()
      break
      ...
      print "\nMoving toward the object, you see that there is a box with a sign next to it.  The sign says IT IS DANGEROUS TO GO ALONE, TAKE ONE OF THESE!  The box is empty."
    elif direction == 'help':
      helpMenu()
    elif direction == "bk":
      if key2Obtained:
        ...
        print "\nI am afraid you can't go back.  You must move forward"
    elif direction == "exit":
      printNow("\nExiting without finishing the game?!\n")
      exitFlag = true
      break
    else:
      print "\nThat is not a direction, insolent fool!"
  #end while
...
def roomthree(room):
  ...
  while userInput != 'exit':
    ...
    userInput = raw_input("Enter command - fd, lt, rt, bk, op: ")
    
    #move commands
    if userInput == "fd":
      if y < 4:
        y = y + 1
        print("You move forward one space. ")
      elif y == 4:
      ...
      if y > 0:
        y = y - 1
        print("You move backward one space. ")
      elif y == 0:
        print("You can't move any further in that direction. ")  
      ...
      if x < 2:
        x = x + 1
        print("You move right one space. ")
      elif x == 2:
        print("You can't move any further in that direction.  ")
    ...
    #open commands, used only for opening doors
      ...
      if x == 1 and y == 4:
        print("You have opened the door to a room that looks like a shrine. ")#take you to room 5
        ...
        print("You have opened the door to the Main Lobby. ")#take you to room 1
        #reply = requestString("Do you wish to walk through the door? ")
        reply = raw_input('Do you wish to walk through the door? ')
        if reply == 'yes' or reply == 'y':
          #nextRoom = 1
...
def map(a,b):
  ...
  #print the map
  print "* * * * * *"#print a * border around the top of the map
  for row in reversed(mapMatrix):
    print " ".join(["*"]+row+["*"])#add *'s on the sides of the map
  print "* * * * * *"#print a * border around the bottom of the map
###################################################################################################################
...
def secretRoom():
  printNow("\n------Secret Room------")
  print "You have entered the secret room and were teleported out of this hell hole. Congrats!"
  sys.exit()
