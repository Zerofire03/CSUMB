...
def enter():
    enterhouse = requestString("Do you want to play a game?  Great rewards lie inside! - press 'y' to continue: ")
...
#Win function        
def leftHouse():
  printNow("You made it out alive. Congrats!")
...
def roomone():
  ...
  while true:
    ...
    if direction == "rt":
      ...
      printNow ("Moving toward the object, you see that there is a box with a sign next to it.  The sign says IT IS DANGEROUS TO GO ALONE, TAKE ONE OF THESE!  The box is empty.")
    elif direction == 'help':
      helpMenu()
    elif direction == "bk":
    ################################################################################## Lab ->
    ...
    ################################################################################## <-Lab 12
        printNow ("I am afraid you can't go back.  You must move forward")
    elif direction == "exit":
      printNow("Exiting without finishing the game?!\n")
      exitFlag = true
      break
    else:
      printNow ("That is not a direction, insolent fool!")
  #end while
...
def room2():
    ...
    printNow("\n------The Locker Room------")
    printNow("This room has one door which is behind you. There is also a locker in front of you with a lock on it.")
    ...
    while loopControl:
        ...
        #Checks to see if the user entered a valid choice, if not, asks for another choice
            ...
            printNow("I wasn't expecting that.")
            userActivity = requestString("Would you like to inspect the locker further or leave the way you came? ")
    ...
    #Checks to see if the user entered yes to interact with the locker
        ...
        if key1Obtained == true:
            printNow("You have a key that looks like it will fit. You try the key and it unlocks the lock.")
            printNow("Inside the locker you find another key that you take with you.")
            setKey2()
            return room2()
        #If key has not been obtained, cannot open locker and tells user.
        else:
            printNow("There is a lock on the locker. You need a key to unlock it.")
            return room2()
    ########################################################################################################################### <- Lab12
    elif userActivity == 'no' or userActivity == 'n':
        printNow("What to do now?")
        return room2()
########################################################################################        
...
def roomthree(room):
  ...
  while userInput != 'exit':
    ...
    #open commands, used only for opening doors
      ...
      if x == 1 and y == 4:
        printNow("You have opened the door to a room that looks like a shrine. ")#take you to room 5
        ...
        printNow("You have opened the door to the Main Lobby. ")#take you to room 1
        #reply = requestString("Do you wish to walk through the door? ")
        reply = requestString('Do you wish to walk through the door? ')
        if reply == 'yes' or reply == 'y':
          #nextRoom = 1
        ...
        printNow("There is no door to open here. ")
    ...
    #interact commands    
      ...
      if ((x == 0 and y == 0) or x == 1) or (x == 2 and y == 2):
        printNow("There is nothing to interact with. ")
      if x == 2 and y == 2:
    ...
    #give the player a hint      
      ...
      printNow("If you're thirsty, have a drink.  There is a pile of coins on the table.  Rumor has it that strange treasures are known to come out of the soda machine, with a little help...")          
...
def room4():
  ...
  while true:
    ...
    if activity not in d:
        printNow("That wasn't what I was expecting.")
        activity = requestString("Do you want to inspect the mirror further or head back the way you came? ").lower()
  ...
  if activity == 'bk':
    ...
    printNow("Death is your only escape! \m/")
    callQuit()
    ...
    if mystery == 'solved':  
      ...
      printNow("You failed to solve the puzzle.  Your life is over!!!\n")
      # test test
      callQuit()
...
def hangMan():
  printNow ("Upon further inspection of the mirror you see a quote:")
  printNow ("What dosn't kill you only makes you stronger, except for ______, they will definitly kill you.")
  printNow ("What do you suppose will definitly kill you?")
